#!/usr/bin/env bats
# Integration tests for Hank loop execution with response analysis and circuit breaker
...
setup() {
    ...
    # Source the main hank_loop.sh functions
    ...
    export PROGRESS_FILE="$HANK_DIR/progress.json"
    ...
    mkdir -p "$HANK_DIR" "$LOG_DIR" "$DOCS_DIR"
...
@test "analyze_response detects structured HANK_STATUS output" {
...
EOF
    ...
    # Check analysis file in .hank/ subfolder
...
@test "init_circuit_breaker creates state file" {
    ...
    assert_file_exists "$HANK_DIR/.circuit_breaker_state"
...
@test "circuit breaker opens on repeated errors" {
    ...
    local same_error_count=$(jq -r '.consecutive_same_error' "$HANK_DIR/.circuit_breaker_state")
...
@test "circuit breaker logs state transitions" {
    ...
    assert_file_exists "$HANK_DIR/.circuit_breaker_history"

    # Verify it's valid JSON
    jq '.' "$HANK_DIR/.circuit_breaker_history" > /dev/null
}
