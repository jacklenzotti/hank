#!/usr/bin/env bats
# Edge case tests for Hank loop execution
...
@test "analyze_response handles large output file" {
    ...
    assert_file_exists "$HANK_DIR/.response_analysis"
    local output_length=$(jq -r '.analysis.output_length' "$HANK_DIR/.response_analysis")
    [[ "$output_length" -gt 50000 ]]
}

# Edge Case 3: Malformed HANK_STATUS block
...
@test "record_loop_result handles invalid file count gracefully" {
    ...
    # Should not crash
    local state=$(jq -r '.state' "$HANK_DIR/.circuit_breaker_state")
...
@test "circuit breaker handles high loop numbers" {
    ...
    local current_loop=$(jq -r '.current_loop' "$HANK_DIR/.circuit_breaker_state")
...
# Edge Case 8: Multiple HANK_STATUS blocks (malformed)
...
@test "circuit breaker handles corrupted state file" {
    ...
    echo "invalid json{" > "$HANK_DIR/.circuit_breaker_state"
...
@test "analyze_response handles missing git repo" {
    ...
    local files_modified=$(jq -r '.analysis.files_modified' "$HANK_DIR/.response_analysis")
...
@test "circuit breaker recreates corrupted history" {
    ...
    # Corrupt history
    echo "not valid json" > "$HANK_DIR/.circuit_breaker_history"
...
EOF
    ...
    local final_exit_signal=$(jq -r '.analysis.exit_signal' "$HANK_DIR/.response_analysis")
    ...
    local last_exit_signal=$(jq -r '.analysis.exit_signal' "$HANK_DIR/.response_analysis")
...
# Edge Case 24: STATUS=COMPLETE but EXIT_SIGNAL=false conflict in HANK_STATUS
