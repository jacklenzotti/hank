...
#
# Parameters:
#   $1 (label) - Label to filter by (optional, default: "hank-task")
...
#   1 - Error
#
# Note: For full PRD conversion with Claude, use hank-import
...
# convert_prd_with_claude - Full PRD conversion using Claude (calls hank-import logic)
#
# Parameters:
#   $1 (prd_file) - Path to the PRD file
#   $2 (output_dir) - Directory to output converted files (optional, defaults to .hank/)
#
# Outputs:
...
convert_prd_with_claude() {
    local prd_file=$1
    local output_dir="${2:-.hank}"

    # This would call into hank_import.sh's convert_prd function
    ...
    # Check if hank-import is available for full conversion
    if command -v hank-import &>/dev/null; then
        # Use hank-import for full conversion
        # Note: hank-import creates a new project, so we need to adapt
        echo "Full PRD conversion available via: hank-import $prd_file"
...
# =============================================================================
# GITHUB ISSUES SYNC (for --source github mode)
# =============================================================================

# sync_github_issues - Fetch open issues and write to IMPLEMENTATION_PLAN.md
#
# Parameters:
#   $1 (label_filter) - GitHub label to filter by (default: "hank")
#   $2 (plan_file) - Path to write plan (default: .hank/IMPLEMENTATION_PLAN.md)
#
# Returns:
#   0 - Success
#   1 - GitHub unavailable or no issues found
#
sync_github_issues() {
    local label_filter="${1:-hank}"
    local plan_file="${2:-.hank/IMPLEMENTATION_PLAN.md}"

    if ! check_github_available; then
        return 1
    fi

    # Fetch issues sorted by priority labels
    local issues
    issues=$(gh issue list --state open --label "$label_filter" \
        --json number,title,labels --jq '
        sort_by(
            if (.labels | map(.name) | index("hank:next")) then 0
            elif (.labels | map(.name) | index("hank:p1")) then 1
            elif (.labels | map(.name) | index("hank:p2")) then 2
            else 3 end
        ) | .[] | "- [#\(.number)] \(.title)"
    ' 2>/dev/null)

    if [[ -z "$issues" ]]; then
        # No open issues — clear stale plan so Claude doesn't re-work completed tasks
        if [[ -f "$plan_file" ]]; then
            cat > "$plan_file" << EMPTYEOF
# Implementation Plan (synced from GitHub Issues)
<!-- Auto-generated by hank --source github. Do not edit manually. -->

No open issues with label "$label_filter".
EMPTYEOF
        fi
        return 1
    fi
...
# report_to_github - Comment on and update a GitHub issue based on loop results
#
# Parameters:
#   $1 (issue_number) - GitHub issue number (without #)
#   $2 (status) - HANK_STATUS value: IN_PROGRESS, COMPLETE, BLOCKED
#   $3 (summary) - One-line summary from RECOMMENDATION field
#   $4 (loop_count) - Current loop iteration number
#
# Returns:
#   0 - Success
#   1 - GitHub unavailable or invalid issue
#
report_to_github() {
    local issue_number="$1"
    local status="$2"
    local summary="${3:-No summary available}"
    local loop_count="${4:-0}"
...
# extract_issue_number - Extract GitHub issue number from text
#
# Parameters:
#   $1 (text) - Text that may contain issue references like #42 or [#42]
#
# Outputs:
#   Issue number (without #) or empty string
#
extract_issue_number() {
    local text="$1"
    # Match [#123] or #123 patterns
    echo "$text" | grep -oE '#[0-9]+' | head -1 | tr -d '#'
}

# setup_github_labels - Create hank labels on the GitHub repo
#
# Returns:
#   0 - Success (labels created or already exist)
...
setup_github_labels() {
    ...
    gh label create "hank" --color "0E8A16" \
        --description "Managed by Hank" 2>/dev/null || true
    gh label create "hank:next" --color "D93F0B" \
        --description "Next for Hank to work on" 2>/dev/null || true
    gh label create "hank:in-progress" --color "FBCA04" \
        --description "Hank is working on this" 2>/dev/null || true
    gh label create "hank:blocked" --color "E4E669" \
        --description "Hank is blocked" 2>/dev/null || true
    gh label create "hank:p1" --color "B60205" \
        --description "High priority" 2>/dev/null || true
    gh label create "hank:p2" --color "D93F0B" \
...
export -f sync_github_issues
export -f report_to_github
export -f extract_issue_number
export -f setup_github_labels
